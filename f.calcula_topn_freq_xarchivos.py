...
flags.DEFINE_integer('n', None, 'Ingrese nombre archivo a procesar.')

#Lee sys.argv en FLAGS
FLAGS(sys.argv)

# Procesamiento
if  FLAGS.archivos != None:
    string_archivos = FLAGS.archivos
    lista_files = string_archivos.split(",")
else:
    lista_files = ['reut2-000.sgm', 'reut2-001.sgm']
    print('------')
    print('Procesando archivo(s): {} por default.'.format(lista_files))
    print('Incluya la opción "--archivo=<archivo1,archivo2...n> para modificar.\n')
...
# concatena múltiples listas provenientes de n archivos en única lista
# de texto procesado
...
print('Procesando archivo(s): {0}:\nTotal palabras: {1}.\n'.format(lista_files, n_palabras))

# genera dataframe con todas las palabras y sus frecuencias
df = gen_df_frecuencias(lista_texto_procesado)

#Selecciona el top m de palabras más frecuentes
if flag_sin_rank == 1:
    print('Frecuencia de palabras en: {}'.format(lista_files))
    print(df)
else:
    df_top = df[0:top_n]
    print('Ranking top {0} palabras más frecuentes en: {1}'.format(top_n, lista_files))
    print(df_top)